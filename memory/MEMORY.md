...
## Current Phase (Day 35)
v0.26.0. Batch infrastructure shipped: APScheduler BOD 9:31/EOD 4:05 ET auto-fire Mon-Fri.
batch_service.py extracted (148 lines). app.py 536→492 lines (KI-086 partial). Batch status dashboard
in DataProvenance (BatchStatusPanel + IVCoverageGrid). MarketData.app credit tracking live.
Two-arch design locked: Alpaca+MD live hours / IBKR batch-only. 15 ETFs confirmed (SCHB not in app).
Free tier (~33 credits/day vs 100 limit) — staying on Free, monitoring before upgrade.
...
4. Read `docs/status/PROJECT_STATUS_DAY35_SHORT.md` — latest day status snapshot
5. Read `docs/versioned/KNOWN_ISSUES_DAY35.md` — open bugs and severity
...
backend/
  ...
  marketdata_provider.py  DONE (Day 27+35) — OI/volume supplement from MarketData.app REST API. Non-blocking.
                          Credit tracking added Day 35 (X-Api-Ratelimit-Remaining/Consumed). ~33 credits/day vs 100 limit.
...
2. **P1:** Live market test — run Best Setups, verify batch-warmed chain cache hits.
3. **P2:** MarketData.app support reply — check email (likely confirms Greeks on all tiers already).
4. **P3:** KI-086 — move _run_one to best_setups_service.py (app.py 492→~420 lines).
5. **P4:** KI-067 — QQQ sell_put ITM strike fix.
...
- `docs/versioned/KNOWN_ISSUES_DAY35.md`
- `docs/status/PROJECT_STATUS_DAY35_SHORT.md`
