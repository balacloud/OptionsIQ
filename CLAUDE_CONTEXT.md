...
> **Project Phase:** IBKR scanner data integration complete. KI-101 resolved (IV/HV live batch + screenshot cache). 44 tests. Next: live test scanner, paper trade logging (0/30), full audit sweep.
...
4. `docs/status/PROJECT_STATUS_DAY50_SHORT.md` — latest day status (update filename each day)
5. `docs/versioned/KNOWN_ISSUES_DAY50.md` — open bugs and severity (update filename each day)
...
| ibkr_provider.py | UPDATED (Day 50) | + get_iv_hv_batch(): reqMktData ticks 104/106/29/30 for all ETFs in one batch. try-finally cancelMktData. |
| scanner_service.py | NEW (Day 50) | get_scanner_data() file cache (4h TTL), fetch_live_iv_hv_batch() IBWorker wrapper, scanner_cache_age_hours(). |
...
## Next Session Priorities (Day 51)

### P0 — Test scanner integration live
Start backend with IB Gateway connected. Run Best Setups. Verify IV/HV column populates from live IBKR batch (tick 106). Check backend.log for `fetch_live_iv_hv_batch` success/failure. ~15 min.

### P1 — Paper trade logging (user action)
Log next XLF or QQQ CAUTION setup to Paper Trade Dashboard. Need 30 trades for win rate data. Still at 0. Cannot calibrate gates without this sample.

### P2 — MASTER_AUDIT_FRAMEWORK sweep
Overdue since Day 42 (8 sessions). Run all 10 categories. Focus on Category 10 (Trading Effectiveness) and gate calibration.

### P3 — Put/call ratio sentiment gate
IBKR ticks 29/30 (callVolume/putVolume) now flow through `live_scanner` in best_setups_service. Wire put/call ratio as a lightweight sentiment overlay (non-blocking warn when >1.5 for sell_put). ~20 lines in gate_engine + constants.

### P4 — KI-099: bull_call_spread direction
...
### Reference
- `docs/versioned/KNOWN_ISSUES_DAY50.md` — current issue list
- `docs/status/PROJECT_STATUS_DAY50_SHORT.md` — Day 50 summary
