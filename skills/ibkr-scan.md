# /ibkr-scan — IBKR Watchlist Morning Scan (MCP + Screenshot)

**Always invoked with the Options_IQ_Claude watchlist screenshot attached.**

Two data sources working together:
- **MCP (12 calls):** IV%, HV%, IVR percentile (13w/26w/52w), P/C ratio, EMA from 502-bar history, range position
- **Screenshot:** OPT. IMP. VOL. CHANGE per ETF (IV direction today — MCP cannot provide this) + 52W IV RANK

Together they restore the full 7-layer scoring including Layer 3 IV Direction, which was the
timing signal: is IV still expanding, or has it peaked and ready to sell?
...
In that case: skip Layer 3 and Layer 4. Note "pre-market — volume/IV-change signals unavailable."

---

## Step 0b — Read the Screenshot

The screenshot shows the Options_IQ_Claude watchlist. Read these two columns per ETF:

| Column | What it shows | Used for |
|--------|--------------|---------|
| **OPT. IMP. VOL. CHANGE** | IV change vs yesterday (e.g. +0.545) | Layer 3 — IV direction |
| **52 WEEK IV RANK** | Raw rank 0–100 (e.g. 44) | Displayed alongside MCP percentile |

Extract per ETF:
```
IV_chg    = OPT. IMP. VOL. CHANGE column value (signed float)
IVR_rank  = 52 WEEK IV RANK column value (integer)
```

Pre-market: if all OPT. IMP. VOL. CHANGE values show 0 or "—" → skip Layer 3, note it.
The screenshot also cross-checks MCP numbers — if P/EMA200 or IV/HV diverge significantly, note it.
...
### Layer 3 — IV Direction (from screenshot)

Is IV still expanding today, or has it peaked? Timing signal — don't sell into expanding vol.

| Condition | Points | Note |
|-----------|--------|------|
| IV_chg ≤ 0 | +1 | IV cooling or contracting — strong sell window, premium at/past peak |
| IV_chg 0.01 to +1.0 | 0 | IV ticking up — acceptable, note it |
| IV_chg > +1.0 | -1 | IV expanding — wait for peak before selling |
| Skip if pre-market (all IV_chg = 0 or "—") | — | Note "pre-market — IV change unavailable" |

**Also output per ETF:** `IV Rank: [IVR_rank] (watchlist) vs [IVR_52w]% (MCP percentile)`
If rank and percentile tell different stories (e.g. Rank=44 but Perc=76%), note it — rank
compresses outliers, percentile is more stable. Percentile is the gate signal; rank is context.
...
```
IBKR SCAN — [Date] [Market hours] [MCP + screenshot]

REGIME: [BULL/BEAR] — SPY P/EMA(200) = [value]

| ETF  | Rank/Perc  | IV/HV | Edge  | IV Chg | P/EMA200 | P/EMA50 | P/C  | Score | Verdict                          |
|------|------------|-------|-------|--------|----------|---------|------|-------|----------------------------------|
| SPY  | 17 / 36%   | 1.30x | +4.2% | +0.22  | +12.09%  |  +5.59% |  —   |   —   | Regime anchor BULL ✅            |
| QQQ  | 44 / 76%   | 1.30x | +4.9% | +0.55  | +20.49%  | +10.04% | 0.99 |  6/7  | SELL PUT ✅                      |
| IWM  | 29 / 53%   | 1.21x | +2.9% | +0.25  | +14.46%  |  +5.87% | 0.79 |  5/7  | SELL PUT (watch DTE)             |
| XLF  | 29 / 67%   | 1.24x | +3.5% | +0.10  |  -0.34%  |  -0.01% | 1.17 |  0/7  | HARD BLOCK ❌ below 200 EMA      |
| TQQQ | 48 / 76%   | 1.28x | +5.0% | +1.05  | +59.28%  | +29.01% | 0.86 |  3/7  | MARGINAL — IV expanding (>1.0)   |
| GLD  | 20 / 47%   | 0.89x | -2.2% | -0.91  |  +3.05%  |  -2.99% | 0.51 |  0/7  | HARD BLOCK ❌ IV cheap (HV > IV) |

Rank/Perc: [52W IV RANK from screenshot] / [IVR_52w percentile from MCP]

IVR multi-window (MCP):
  QQQ: 13w=xx% / 26w=xx% / 52w=76%
  IWM: 13w=xx% / 26w=xx% / 52w=53%

IV Direction (Layer 3 — from screenshot):
  QQQ:  +0.55 → IV ticking up, acceptable
  IWM:  +0.25 → IV ticking up, acceptable
  TQQQ: +1.05 → ⚠️ IV EXPANDING — wait for peak before opening satellite
  GLD:  -0.91 → IV contracting (blocked anyway)

Volume conviction (MCP):
  QQQ: calls xx% of avg · puts xx% of avg

Price extension (MCP):
  QQQ: range_pos=xxx% — [ATH note if applicable]

TOP PICK: QQQ sell_put
Rationale: IVR_52w 76%, IV/HV 1.30x (+4.9% edge), uptrend both EMAs, P/C 0.99 call-dominant.

[System warnings if any]

━━━ SCAN CONTEXT — copy and paste into OptionsIQ analyze tool ━━━
TICKER=QQQ  IVR=76  IV_HV=1.301  PC=0.99  PEMA200=+20.49  PEMA50=+10.04  DIRECTION=sell_put
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
