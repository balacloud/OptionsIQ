...
> **Usage:** `/catalyst-check [ticker] [dte] [direction] [strike]`
> **Example:** `/catalyst-check XLF 35 sell_put $49`
...
## Machine Block (paste into OptionsIQ CATALYST CONTEXT field)

After the human-readable output above, always emit this line at the very end:
...
**Rules:**
- Omit `HOLDINGS_COMPANY` and `HOLDINGS_DAYS` when `HOLDINGS_RISK=false`
- `FOMC_DAYS` = days until nearest FOMC in window (0 if none → use 999)
- `FOMC_TIER` from Step 3a tier logic: XLF/TQQQ/XLRE ≤14d → `block`; QQQ/IWM/GLD → `warn`; all others → `pass`
- Map verdict: PROCEED → `proceed`, CAUTION → `caution`, ABORT → `abort`
- `MACRO_COUNT` = total CPI + NFP + PCE events falling inside the DTE window
