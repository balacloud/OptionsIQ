...
- [x] Skew computation — `compute_skew()` in tradier_provider.py, put_iv_30d − call_iv_30d via 30-delta, exposed in analyze response ✅ Day 42
- [x] KI-064: IVR mismatch L2/L3 — `_extract_iv_data()` uses ATM contract IV, fallback to average ✅ Day 43
- [x] KI-075: GateExplainer GATE_KB drift — hv_iv_vrp + vix_regime entries added; ETF sell_put DTE gate uses correct ETF_DTE_SELLER_PASS_MIN/MAX ✅ Day 43
- [x] KI-077: DirectionGuide sell_put risk label — honest text (naked put risk) ✅ Day 43
- [x] KI-081: Macro events calendar (CPI/NFP/PCE) — MACRO_DATES + _days_until_next_macro() + gate extended; fomc_days_away silent 999 bug fixed ✅ Day 43
- [x] app.py size violation (KI-086) — sta_service.py extracted Day 46 (sta_fetch 74-line inline logic → fetch_sta_swing_data()). app.py 472→402 lines. All business logic now in service modules. ✅ Day 46
...
- [x] MarketData.app greeks pipeline — IV+delta+gamma+theta+vega surfaced through provider; IV patches IVR gate when Alpaca IV=null ✅ Day 36
...
- [x] Tradier integration — `tradier_provider.py` created, wired as primary live chain source (IBKR removed from DataService live path) ✅ Day 39
- [x] KI-086: best_setups_service.py extracted — `_run_one` → `run_one_setup()`, app.py 497→449 lines ✅ Day 39
- [x] KI-067: QQQ sell_put ITM fix — ibkr_provider OTM filter + strategy_ranker fallback → return [] ✅ Day 39
- [x] ARCH_DECISION_TRADIER_PRIMARY.md + backup — documented revert path for Tradier primary change ✅ Day 39
- [x] KI-090: Tradier delta=0.0 coercion bug — `float(g[key]) if g.get(key) is not None else None` ✅ Day 40
...
| Version | Day | Notes |
|---------|-----|-------|
| v0.36.2 | Day 70 | **Quick Reference card in Learn tab.** New 6th panel — strategy comparison table, call buying R1/R2/R3 ranking (δ0.68/0.50/0.30), key thresholds (DTE/IVR/GLD/TQQQ/exit rule), 100% loss warning box. Build clean. 110 tests. |
...
| v0.31.0 | Day 48 | **Phase 7c external audit + STA code review (no code changes).** STA backend.py reviewed: RS ratio = midpoint normalization (swing-trading RRG variant), weekChange returned but unused by quadrant_to_direction(). Phase7c_Research.md consolidated from two files. ChatGPT 4-question audit stored: 5 design gaps → KI-096 (IVR null coercion), KI-097 (event density), KI-098 (absolute trend gate), KI-099 (bull_call_spread direction), KI-100 (Tier 1 GO rate). Awaiting Gemini/Perplexity before implementing. |
| v0.31.0 | Day 47 | **README full rewrite + Phase 7c trading effectiveness improvements.** README rewritten fixing 9+ stale sections. ETF_OPTIONS_LIQUID_TIER1 constant (QQQ/IWM/XLF/XLK/XLY). _is_early_market_session() helper. Actionable liquidity gate messages: Tier2 ETFs get "try QQQ/XLF", early session gets "rescan after 10 AM". Phase7c_Research.md (merged from Day46+47 docs). |
| v0.30.1 | Day 46 | **KI-086 closed + DTE calibration hardening.** sta_service.py extracted (sta_fetch 74 lines → 2 lines, app.py 472→402). Pre-existing ETF sell_call DTE bug fixed: _run_etf_sell_call() created (was using stock DTE constants 14-21, ETF should use 30-45). ETF_DTE_SELLER_PASS_MIN 21→30 (tastylive 200k+ trade research). Category 10 live scan: 2/11 CAUTION, Liquidity Proxy dominant blocker, vol gates all pass VIX=17.39. |
| v0.30.0 | Day 45 | **Framework/docs session.** MASTER_AUDIT_FRAMEWORK v1.4: Category 10 (Trading Effectiveness Audit) added — 5 checks: gate pass rate, "always one direction", DTE calibration, unbiased evaluation, expected value sanity. Phase 7c scope expanded in ROADMAP. README rewrite deferred. No code changes. |
...
| v0.27.0 | Day 37 | Startup catch-up (missed BOD/EOD jobs). yfinance HV removed from IV seeding (HV≠IV contamination fix). Research folder reorganized. Tradier confirmed free. |
| v0.26.1 | Day 36 | MarketData.app greeks pipeline — IV+delta+gamma+theta+vega surfaced. IV patches IVR gate when Alpaca chain IV=null (iv_source="marketdata"). md_supplement in analyze response. |
